...
and Horizontal = { Y: int; StartX: int; EndX: int; Steps: int }
and Vertical = { X: int; StartY: int; EndY: int; Steps: int }
...
type Solver() =
                    ...
                    | (direction, distance)::tl -> 
                        let nextPosition = 
                            match direction with
                                | 'U' -> { position with Y = position.Y + distance; Steps = position.Steps + distance }
                                | 'D' -> { position with Y = position.Y - distance; Steps = position.Steps + distance }
                                | 'L' -> { position with X = position.X - distance; Steps = position.Steps + distance }
                                | 'R' -> { position with X = position.X + distance; Steps = position.Steps + distance }
    ...
    let toLine (p1: Point, p2: Point) = 
                ...
                | _ when x1 <> x2 -> Horizontal { StartX = x1; EndX = x2; Y = y1; Steps = p1.Steps}
                | _ when y1 <> y2 -> Vertical { StartY = y1; EndY = y2; X = x1; Steps = p1.Steps}
    ...
    let getIntersection (h: Horizontal) (v: Vertical) =
            ...
            if intersects then
                let sumSteps = h.Steps + abs (v.X - h.StartX) + v.Steps + abs (h.Y - v.StartY)
    ...
    interface Shared.ISolver with
        member this.PartOne(input: string): obj =
            let wires = 
                ...
                |> List.map 
                    (getPoints >> List.pairwise >> List.map toLine)
            ...
            intersections |> Seq.map (fun intersection -> abs intersection.X + abs intersection.Y) |> Seq.min  |> box
        ...
        member this.PartTwo(input: string): obj =
            ...
            intersections |> Seq.map (fun intersection -> intersection.Steps) |> Seq.min |> box
