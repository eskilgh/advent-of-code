...
internal class Solver : ISolver
{
    ...
    private static bool IsPartNumber(int rowPos, int colPos, int length, string[] lines)
    {
        ...
        if (rowPos > 0)
        {
            ...
            {
                if (IsSymbol(lines[rowPos - 1][col]))
                    return true;
            }
        }
        if (colPos > 0)
        {
            if (IsSymbol(lines[rowPos][colPos - 1]))
                return true;
        }
        if (colPos + length < lines[0].Length)
        {
            if (IsSymbol(lines[rowPos][colPos + length]))
                return true;
        }
                ...
                (line, row) =>
                    line.Select(
                        (c, col) =>
                        {
                            ...
                            var adjacentNumbers = nums.Where(num =>
                                {
                                    return Math.Abs(row - num.Row) <= 1
                                        && num.Col - 1 <= col
                                        && col <= num.Col + num.Length;
                                })
                                .ToList();

                            return adjacentNumbers.Count == 2 ? adjacentNumbers[0].Value * adjacentNumbers[1].Value : 0;
                        }
                    )
            )
            .Sum()
