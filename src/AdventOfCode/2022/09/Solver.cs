...
internal class Solver : ISolver
{
  ...
  private static IEnumerable<(string Direction, int Steps)> ParseInstructions(string input) => input
      .Split("\n")
  ...
  private static List<Vector2> PerformStep(List<Vector2> knot, string direction)
  {
    ...
    return knot
       .TakeLast(knot.Count - 1)
       .Aggregate(new List<Vector2>
         { knot[0] + directionVector },
         (knot, current) =>
           knot.Append(MoveTail(knot[^1], current)).ToList()
       );
  }
...
file static class Extensions
{
  ...
  public static Vector2 ToVector(this string direction) => direction switch
  {
    "U" => new Vector2(0, 1),
    "L" => new Vector2(-1, 0),
    "D" => new Vector2(0, -1),
