...
internal class Solver : ISolver
{
  public string PartOne(string input)
  {
    ...
    var start =
      (from row in heightMap.Select((cs, i) => (cs, i))
       ...
       where col.c == 'S'
       select (col.c, row.i, col.j)).First();
  ...
  {
    var steps = Enumerable.Range(0, heightMap.Length)
      .Select(_ => Enumerable.Range(0, heightMap[0].Length)
        .Select(_ => -1).ToArray())
      .ToArray();
  ...
  private static IList<(int RowInc, int ColInc)> Directions =>
     new List<(int RowInc, int ColInc)>
    {
      (-1, 0),
      (0, 1),
      (1, 0),
  ...
  {
    return Directions
      .Select(d => current with
      {
        Row = current.Row + d.RowInc,
        Col = current.Col + d.ColInc
      })
      .Where((pos) =>
        0 <= pos.Row && pos.Row < heightMap.Length &&
        0 <= pos.Col && pos.Col < heightMap[0].Length)
      .Where((pos) =>
      {
        var currentElevation = GetElevation(heightMap, current);
        var newPosElevation = GetElevation(heightMap, pos);
        return ascending ? (newPosElevation - currentElevation <= 1) :
