...
internal class Solver : ISolver
{
  public string PartOne(string input)
  {
    ...
    return root
      .Where(entry => entry is Directory && entry.Size < 100000)
      .Sum(dir => dir.Size).ToString();
  }
  ...
  public string PartTwo(string input)
  {
    ...
    return root
      ...
      .Aggregate(int.MaxValue, (candidate, current) =>
         current.Size < minCandidateSize
          ? candidate
          : Math.Min(current.Size, candidate)
      ).ToString();
  }
...
{
  public override int Size { get => FileSize; }
